    ...
    with db() as conn:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                total_quizzes_taken INTEGER DEFAULT 0, total_correct INTEGER DEFAULT 0, total_wrong INTEGER DEFAULT 0
            );
...
def finish_quiz(session_id):
    with db() as conn:
        ...
        if quiz:
            cur.execute("UPDATE users SET total_quizzes_taken=total_quizzes_taken+1, total_correct=total_correct+%s, total_wrong=total_wrong+%s WHERE user_id=%s",
...
📄 <b>Fayl formati:</b>
━━━━━━━━━━━━━━━
Faylni ChatGPT yoki boshqa AI ga shunday deb yuboring:
<i>"Menga quyidagi formatda .docx quiz fayli yarat: # QUIZ: [mavzu nomi], keyin Q1, Q2... shaklida savollar, har birida A) B) C) D) variantlar va ANSWER: [harf] javob"</i>
...
📌 <b>Qoidalar:</b>
━━━━━━━━━━━━━━━
• <code># QUIZ:</code> — quiz nomi (faqat bir marta)
• <code>Q1, Q2, Q3...</code> — savollar ketma-ketligi
• Faqat <code>A) B) C) D)</code> variantlar
• <code>ANSWER:</code> — faqat bitta harf (A/B/C/D)
• Har bir savoldan keyin bo'sh qator bo'lishi shart
• Fayl <b>.docx</b> formatida bo'lishi kerak
...
📄 <b>Формат файла:</b>
━━━━━━━━━━━━━━━
Отправьте ChatGPT или другому AI такой запрос:
<i>"Создай .docx файл квиза в формате: # QUIZ: [тема], затем Q1, Q2... с вариантами A) B) C) D) и ANSWER: [буква]"</i>
...
📌 <b>Правила:</b>
━━━━━━━━━━━━━━━
• <code># QUIZ:</code> — название (один раз)
• <code>Q1, Q2, Q3...</code> — порядок вопросов
• Только <code>A) B) C) D)</code>
• <code>ANSWER:</code> — одна буква (A/B/C/D)
• Пустая строка после каждого вопроса
• Файл должен быть в формате <b>.docx</b>
...
📄 <b>File format:</b>
━━━━━━━━━━━━━━━
Send this prompt to ChatGPT or any AI:
<i>"Create a .docx quiz file in this format: # QUIZ: [topic], then Q1, Q2... with options A) B) C) D) and ANSWER: [letter]"</i>
...
📌 <b>Rules:</b>
━━━━━━━━━━━━━━━
• <code># QUIZ:</code> — quiz name (once only)
• <code>Q1, Q2, Q3...</code> — question order
• Only <code>A) B) C) D)</code> options
• <code>ANSWER:</code> — one letter only (A/B/C/D)
• Empty line after each question
• File must be in <b>.docx</b> format
...
def menu_kb(user_id):
    ...
    rows = [
        ...
        [KeyboardButton(t(user_id, 'menu_stats')), KeyboardButton(t(user_id, 'menu_lang'))],
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If new user - show lang selection first
    if not existing:
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    u = get_user(uid)
    if not u: return
    c = u['total_correct'] or 0
    w = u['total_wrong'] or 0
    total = c + w
    p = round(c/total*100 if total > 0 else 0, 1)
    await update.message.reply_text(t(uid, 'stats', q=u['total_quizzes_taken'] or 0, c=c, w=w, p=p),
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Menu buttons
    if "Quiz boshlash" in text: await cmd_quiz(update, context)
    elif "Holat" in text: await cmd_status(update, context)
    elif "Statistika" in text: await cmd_stats(update, context)
    elif "Til" in text: await cmd_lang(update, context)
    elif "Yordam" in text: await cmd_help(update, context)
...
def main():
    ...
    if WEBHOOK_URL:
        ...
        logger.info("🚀 Polling mode (webhook URL yo'q)")
        app.run_polling(
            drop_pending_updates=True,
            timeout=60,
            read_timeout=60,
